...
class Match(models.Model):
    ...
    player1 = models.ForeignKey(
        "account.ProfileInCup",
        on_delete=models.CASCADE,
        related_name="player1",
        verbose_name="Gracz 1",
    )
    player2 = models.ForeignKey(
        "account.ProfileInCup",
        on_delete=models.CASCADE,
        related_name="player2",
        verbose_name="Gracz 2",
    )
    result1 = models.PositiveSmallIntegerField(
        verbose_name="Wynik gospodarza", default=None, null=True
    )
    result2 = models.PositiveSmallIntegerField(
        verbose_name="Wynik gościa", default=None, null=True
    )
    cup = models.ForeignKey(
        "Cup", on_delete=models.CASCADE, related_name="cup_in_match", default=""
    )
    round = models.ForeignKey(
        "Round", on_delete=models.CASCADE, related_name="round", default=""
    )
...
class Cup(models.Model):
    ...
    name = models.CharField(
        max_length=50, blank=False, verbose_name="Nazwa", default=""
    )
    number_of_players = models.PositiveSmallIntegerField(
        verbose_name="Liczba graczy",
        default=0,
    ...
    types = (
        ("Puchar", (("Puchar", "Puchar (min. 4 graczy)"),)),
        (
            "Liga",
            (
    ...
    type = models.CharField(
        max_length=20, choices=types, default=types[0][0], verbose_name="Rodzaj"
    )
    reg_status = (
        ("Otwarta", "Otwarta"),
        ("Zamknięta", "Zamknięta"),
    ...
    registration = models.CharField(
        max_length=20,
        choices=reg_status,
        default=reg_status[0][0],
        verbose_name="Rejestracja",
    )
    len_rounds = models.PositiveSmallIntegerField(verbose_name="Liczba rund", default=0)
    elimination_matches = models.PositiveSmallIntegerField(
        verbose_name="Liczba meczy eliminacyjnych", default=0
    )
    elimination_generated = models.BooleanField(
        verbose_name="Wygenerowano emilimacje", default=False
    )
    league_generated = models.BooleanField(
        verbose_name="Wygenerowano ligę", default=False
    )
    actual_round = models.ForeignKey(
        "Round",
        on_delete=models.CASCADE,
        related_name="actual_round",
        default=None,
        null=True,
    )
    ...
    players_order = models.CharField(
        max_length=50, blank=False, verbose_name="Kolejność Graczy", default=""
    )
    online = models.BooleanField(verbose_name="online?", default=False)
    declarations_status = (
        ("Offline", (("Ręczna", "Ręczna (organizator)"),)),
        (
            "Online",
            (
                ("Otwarta", "Otwarta"),
    ...
    declarations = models.CharField(
        max_length=20,
        choices=declarations_status,
        default=declarations_status[0][0],
        verbose_name="Rejestracja",
    )
    players = models.ManyToManyField(User, related_name="players")
    groupanothercup = models.ForeignKey(
        "Cup",
        on_delete=models.CASCADE,
        related_name="group_cups",
        default=None,
        null=True,
    )
    promotion = models.ManyToManyField(
        "account.ProfileInCup",
        related_name="promotion_group",
        verbose_name="Awans",
        blank=True,
    )
    cupgenerated = models.BooleanField(
        verbose_name="Wygenerowano puchar", default=False
    )
...
class Round(models.Model):
    ...
    name = models.CharField(
        max_length=50, blank=False, verbose_name="Nazwa rundy", default=""
    )
    cup = models.ForeignKey(
        "Cup", on_delete=models.CASCADE, related_name="cup_in_round", default=""
    )
    promotion = models.ManyToManyField(
        "account.ProfileInCup",
        related_name="promotion_round",
        verbose_name="Awans",
        blank=True,
    )
    ...
    pausing = models.ForeignKey(
        "account.ProfileInCup",
        on_delete=models.CASCADE,
        verbose_name="Pauzuje",
        null=True,
    )
...
class Invite(models.Model):
    ...
    status = models.CharField(
        max_length=20, choices=stat, default=stat[0][0], verbose_name="Status"
    )
